export const euro=n=>Number.isFinite(n)?new Intl.NumberFormat('es-ES',{style:'currency',currency:'EUR',maximumFractionDigits:0}).format(n):'N/D';
...
export const nav=`<div class="top"><a class="brand" href="${appUrl('executive-summary.html')}">Investment Decision Lab <span class="tag">V4</span></a><nav class="nav"><a href="${appUrl('executive-summary.html')}">Resumen</a><a href="${appUrl('entender-inversiones.html')}">Entender las inversiones</a><a href="${appUrl('compare.html')}">Comparador</a><a href="${appUrl('projections.html')}">Proyecciones</a><a href="${appUrl('analytics.html')}">Analítica</a><a href="${appUrl('alternatives.html')}">Alternativas 45k</a><a href="${appUrl('risk-register.html')}">Riesgo</a><a href="${appUrl('data-quality.html')}">Datos</a><a href="${appUrl('report.html')}">Informe</a></nav></div>`;
const dataCache=new Map();
export async function load(path){
  if(dataCache.has(path))return dataCache.get(path);
  const request=(async()=>{const url=appUrl(path),response=await fetch(url);if(!response.ok)throw new Error(`No se pudo cargar ${path} (HTTP ${response.status})`);try{return await response.json()}catch{throw new Error(`El recurso ${path} no contiene JSON válido`)}})();
  dataCache.set(path,request);
  try{return await request}catch(error){dataCache.delete(path);throw error}
}
